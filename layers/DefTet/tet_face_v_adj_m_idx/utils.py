...
tet_face_v_adj_m_idx = load(name="tet_face_v_adj_m_idx",
...
class VarianceFunc(Function):
    ...
    def forward(ctx, face_fx3x3):
        ...
        n_point = n_face * 3
        adj_idx = -1 * torch.ones(n_point, n_max_nei, device=face_fx3x3.device).float() # each face can only have maximum 10 neighbor face
        tet_face_v_adj_m_idx.forward(face_fx3x3, adj_idx)

        # import ipdb
        # ipdb.set_trace()
        idx = torch.arange(0, n_point, device=face_fx3x3.device, dtype=torch.long).int()
...
###############################################################
tet_face_v_adj_m_f_idx = VarianceFunc.apply
