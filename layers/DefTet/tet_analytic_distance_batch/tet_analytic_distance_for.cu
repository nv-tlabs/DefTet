...
__global__ void dr_cuda_forward_kernel_batch_min_distance(
		...
		scalar_t* __restrict__ n_face_b,
		int n_batch, int n_point, int n_max_tet)
		{
	...
	int tet_base_idx = batch_idx * n_max_tet * 3 * 3;
	...
	scalar_t face[9];
	int n_tet_face = static_cast<int>(n_face_b[batch_idx]);
	for(int i_face = 0; i_face < n_tet_face; i_face++){
	    int base_face = tet_base_idx + i_face * 3 * 3;
...
void dr_cuda_forward_batch(at::Tensor gt_point_clouds_bxpx3, at::Tensor face_bxfx3x3, at::Tensor closest_f,
                            at::Tensor closest_d, at::Tensor n_face_b){

	int n_batch = gt_point_clouds_bxpx3.size(0);
	int n_point = gt_point_clouds_bxpx3.size(1);
	int n_max_tet = face_bxfx3x3.size(1);
	...
	AT_DISPATCH_FLOATING_TYPES(gt_point_clouds_bxpx3.type(), "dr_cuda_forward_kernel_min_distance", ([&] {
		dr_cuda_forward_kernel_batch_min_distance<scalar_t><<<blocks, threads>>>(
				...
				n_face_b.data<scalar_t>(),
				n_batch, n_point, n_max_tet);
	}));
